...
const WatchMovie: React.FC<WatchMovieProps> = ({
  ...
  const fetchTrailer = useCallback(async (title: string, releaseDate: string): Promise<TrailerData | null> => {
    ...
    if (!YOUTUBE_API_KEY || YOUTUBE_API_KEY !== 'AIzaSyBGOViLmJSgDLXBIBrb7jpGscpJlUeopd0') {
      const mockTrailers = [
  ...
  }, [player, isPlayerReady, movieId, title, releaseDate, rating, currentPlayTime, watchHistoryId, getToken, totalDuration, genre]);
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start gap-4">
            <div className="flex-1 space-y-3">
              ...
              <div className="flex items-center space-x-4 text-sm text-gray-300">
                ...
                <span className="bg-green-600 px-2 py-1 rounded text-xs font-bold">4K UHD</span>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <p className="text-gray-300">
                    {genre && genre.length > 0 ? genre.join(', ') : 'Action, Drama'}
                  </p>
                </div>
                <div>
                  <h4 className="text-white font-semibold mb-1">Duration</h4>
                  <p className="text-gray-300">
                    {totalDuration ? `${Math.floor(totalDuration / 60)}h ${totalDuration % 60}m` : '2h 30m'}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-gray-300">Leonardo DiCaprio, Marion Cotillard, Tom Hardy</p>
                </div>
              </div>
            </div>
          )}

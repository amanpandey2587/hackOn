...
const MainContainer = ({ movies, movieDetails, fetchMovieDetails }: MainContainerProps) => {
  ...
  const [isWatchMovieOpen, setIsWatchMovieOpen] = useState<boolean>(false);
  const [tvMazeDetails, setTvMazeDetails] = useState<{[key: string]: TVMazeShow}>({});
  const [loadingTvMaze, setLoadingTvMaze] = useState<{[key: string]: boolean}>({});

  // Fetch additional details from TVMaze API
  const fetchTVMazeDetails = useCallback(async (movieTitle: string, movieYear?: number): Promise<TVMazeShow | null> => {
    ...
    try {
      ...
      const searchResponse = await fetch(`https://api.tvmaze.com/search/shows?q=${encodeURIComponent(movieTitle)}`);
        ...
        if (movieYear) {
          for (const result of searchData) {
            ...
            if (show.premiered) {
              ...
              const currentBestYear = bestMatch.premiered ? new Date(bestMatch.premiered).getFullYear() : 0;
              ...
              if (Math.abs(showYear - movieYear) < Math.abs(currentBestYear - movieYear)) {
                bestMatch = show;
              }
            }
          }
      ...
      console.error('Error fetching TVMaze details for', movieTitle, ':', error);
    } finally {
  ...
  const handleMovieSelection = (index: number) => {
    ...
    if (selectedMovie && !tvMazeDetails[`${selectedMovie.title}-${selectedMovie.year}`]) {
  ...
  const tvMazeData = tvMazeDetails[`${selectedMovie.title}-${selectedMovie.year}`];
  ...
  const getTotalDuration = (): number => {
    if (additionalDetails?.runtime_minutes) return additionalDetails.runtime_minutes;
  ...
  const getOverview = (): string => {
    ...
    if (additionalDetails?.plot_overview) return additionalDetails.plot_overview;
              ...
              <div className="flex-1 text-white">
                ...
                {tvMazeData && (
                  <div className="text-sm text-gray-300 space-y-1">
                    {tvMazeData.status && (
                      <p><span className="font-semibold">Status:</span> {tvMazeData.status}</p>
                    )}
                    {tvMazeData.network?.country?.name && (
                      <p><span className="font-semibold">Country:</span> {tvMazeData.network.country.name}</p>
                    )}
                    {tvMazeData.language && (
                      <p><span className="font-semibold">Language:</span> {tvMazeData.language}</p>
                    )}
                  </div>
                )}
              </div>
